...
namespace Hsf.YunZig
{
    ...
    /// 服务端代码辅助类
    /// </summary>
    public class YuiYingServer
    {
        private static log4net.ILog log = log4net.LogManager.GetLogger("YuiYingServer");
        static int connectCount = 0;
        //static int receiveCount = 0;
        static Encoding encoding = Encoding.ASCII;
        HansAppServer myServer = new HansAppServer(encoding, "2a ", "23 ");//{\"code\":", "}
...
            if (!myServer.Start())
        ...
        private void MyServer_NewRequestReceived(HansAppSession session, HansRequestInfo requestInfo)
        {
            Task.Run(() =>
            {
                ...
                string[] sArray = Regex.Split(msg, "23 2a ", RegexOptions.IgnoreCase);
                for (int i = 0; i < sArray.Length; i++)
                {
                    if (!sArray[i].StartsWith("2a "))
                    {
                        sArray[i] = "2a " + sArray[i];
                    }
                    int _left = Regex.Matches(sArray[i], "2a ").Count;
                    int _right = Regex.Matches(sArray[i], "23 ").Count;
                    int cha = _left - _right;
                    for (int b = 0; b < cha; b++)
                    {
                        sArray[i] = sArray[i] + "23 ";
                    }
